...
const UpdateAppraisalFromStage4 = () => {
  ...
  const updateValues = (e, id, kpi) => {
    ...
    console.log(">>>>>>value", value, kpi, id);

    if (value > Number(kpi.measurableTarget) && kpi.kpiId) {
      value = "";
      setErrors({ ...errors, [kpi.kpiId]: true });
    } else {
      setErrors({ ...errors, [kpi.kpiId]: false });
    }
    ...
    setValues({ ...values, [kpi.kpiId]: value });

    //values for supervisorResults is set
    setAllSupervisorResults({ ...allSupervisorResults, [kpi.kpiId]: result });

    setAppraiseeResults({ ...appraiseeResults, [kpi.kpiId]: result });
  };

  console.log(">>>>>allREsults", allSupervisorResults);
  ...
  const submitAppraisal = () => {
    const appraise = details?.kpis.map((kpi) => {
      return {
        supervisorRate: values[kpi.kpiId],
        key: kpi.key,
        supervisorResult: allSupervisorResults[kpi.kpiId].toFixed(),
    ...
    const appraisals = {
      ...
      supervisorComment: supervisorReply,
                          ...
                          <div className="profile-view">
                            ...
                            <div className="row">
                              ...
                              {/* {allFinancial?.map((kpi) => {
                                return (
                                  <EditSupervisorKpiInputComponent
                                    errors={errors}
                                    kpi={kpi}
                                    values={values}
                                    defaultValue={kpi.supervisorRate}
                                  
                                    defaultRate={kpi.supervisorResult}
...
export default UpdateAppraisalFromStage4;
